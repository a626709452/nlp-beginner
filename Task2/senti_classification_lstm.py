...
# train the model #
# ########################################################### #
W = tf.Variable(tf.random_uniform([hidden_size, CLASS_NUM]), dtype=tf.float32)
bias = tf.Variable(tf.constant(0.1, shape=[CLASS_NUM]), dtype=tf.float32)
y_pre = tf.nn.softmax(tf.matmul(h_state, W) + bias)

# loss function
cross_entropy = -tf.reduce_mean(y * tf.log(y_pre))
train_op = tf.train.AdamOptimizer(learningrate).minimize(cross_entropy)
...
def load_train():
    pd_lines = pd.read_csv("data/train.tsv", header=0, delimiter="\t", quoting=3)
...
# read test.tsv
def load_test():
    pd_lines = pd.read_csv("data/test.tsv", header=0, delimiter="\t", quoting=3)
...
#    X, y: np.array
#    model: class Model
def train_model(trainset, feature, ngram_to_ix, _learning_rate):
    m = len(trainset)
    phrase, sentiment = zip(*trainset)
    _batch_size = 120
    total_size = _batch_size * timestep_size
    for iter in range(m / total_size - 1):
        batch_X = np.array([generate_feature(str, feature, ngram_to_ix) \
                        for str in phrase[iter * total_size : (iter+1) * total_size]])
        batch_y = np.array([generate_label(level) \
                        for level in sentiment[iter * total_size : (iter+1) * total_size]])
        
        sess.run(train_op, feed_dict={_X:batch_X, y:batch_y, batch_size:_batch_size, keep_prob:1, learningrate:_learning_rate})
        p = sess.run(y_pre, feed_dict={_X:batch_X, batch_size:_batch_size, keep_prob:1})
        train_loss = sess.run(cross_entropy, feed_dict={_X:batch_X, y:batch_y, keep_prob:1, batch_size:_batch_size})
        if iter % 100 == 0:
...
if __name__ == "__main__":
    ...
    for epoch in range(200):
        train_model(trainset, args.f, ngram_to_ix, args.l)
